...
function fenomenSil(gelenDizi,index) {
...
1. ilk parametre olarak fenomenler dizisini alın
2. İKİNCİ, ÜÇÜNCÜ, DÖRDÜNCÜ, BEŞİNCİ ve ALTINCI parametre olarak sırasıyla bir sırano (number), bir profil ismi (number), takipçi sayısı (followers), gönderim sayısı  (posts) ve bir platform adı (platform) alın.
...
function fenomenEkle(fenomenler,gelenNumber,gelenprofil,gelenfollowers,gelenposts,gelenplatform) {
  ...
  console.log(fenomenEkle(fenomenler,1250,"Hilal",777,5555,"İnstagram"));
/* Görev 7:
...
function enFenomenler(dizi) {
  ...
  for(let i=0;i<dizi.length;i++){
    if(dizi[i].followers>=100000000){
      yeniDizi.push(dizi[i].followers)
...
function fenomenGonderimSayisi(dizi,fenomenName){
  let x =[]
  for(let i=0;i<dizi.length;i++){
    if(dizi[i].profile===fenomenName){
...
for(let i=0;i<dizi.length;i++){
  let feno=dizi[0].profile;
  let max=dizi[0].posts
  for(let i=0;i<dizi.length;i++){
   if(dizi[i].platform===platform && dizi[i].posts>max){
    max=dizi[i].posts;
    feno=dizi[i].profile
   } 
  }
